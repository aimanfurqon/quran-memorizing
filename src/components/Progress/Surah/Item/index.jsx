...
import Content from "assets/mushaf/juz25";

const ProgressSurahItem = ({
  surah,
  activeSurah = false,
  setActiveSurah,
  navigation,
  ...
  const navigateToSurah = (targetType = "surah", ayahNumber = null) => {
    if (!surahContent?.hasOwnProperty("page"))
      return showMessage({
        message: "Halaman yang diminta belum tersedia saat ini.",
        type: "warning",
        color: "#472a00",
      });
    const content = Content()["metadata"].find(
      (item) => String(item.number) === String(surahContent.no)
    );
    const pageTarget =
      targetType === "surah"
        ? surahContent?.page
        : content["ayah"][ayahNumber - 1]["pageIndex"];
    return navigation.navigate("Mushaf", {
      pageIndex: Number(pageTarget),
      activeAyah:
        targetType === "surah" ? null : `${surahContent.no}:${ayahNumber}`,
    });
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <View
            style={{
              height: "100%",
              width: `${(memorized / surah.item.numberOfAyah) * 100}%`,
              borderRadius: 999,
            }}
          >
            <LinearGradient
      ...
      {active && (
        ...
        >
          {Array.from(
            { length: Number(surahContent?.numberOfAyah) },
            (_, i) => i + 1
          ).map((ayahItem) => {
